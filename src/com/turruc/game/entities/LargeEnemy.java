	...
	public void update(GameContainer gc, float dt) {
		...
		for (int y = 0; y < 2; y++) {
			for (int x = 0; x < 2; x++) {
				...
				if (new LOSBullet((int) player.getPosX(), (int) player.getPosY(), tileX + x, tileY + y, offX, offY, GameManager.getGc(), GameManager.getGc().getDt(), range, true, true).LOS) {
					// slow motion
					...
					// lava damage
					if (GameManager.gm.getCollisionNum((int) tileX + x, (int) tileY) + y == 3 && (System.nanoTime() / 1000000000.0) - lastTimeLavaDamage > lavaDamageCooldown) {
					...
					if (GameManager.gm.getPlayer().getPosX() > this.posX) {
						if (GameManager.gm.getCollision(tileX + 1 + x, tileY + y) || GameManager.gm.getCollision(tileX + 1 + x, tileY + y + (int) Math.signum((int) offY))) {
							offX += dt * speed;
							if (offX > padding) {
					...
					if (GameManager.gm.getPlayer().getPosX() < this.posX) {
						if (GameManager.gm.getCollision(tileX - 1 + x, tileY + y) || GameManager.gm.getCollision(tileX + x - 1, tileY + y + (int) Math.signum((int) offY))) {
							offX -= dt * speed;
							if (offX < -padding) {
					...
					if (ground && (againstWall || GameManager.gm.getCollisionNum((int) tileX + x, (int) tileY + y) == 3)) { // makes
																													// him
																													// jump
																													// if
																													// hes
					...
					if (fallDistance < 0) {
						if ((GameManager.gm.getCollision(tileX + x, tileY + y - 1) || GameManager.gm.getCollision(tileX + x + (int) Math.signum((int) Math.abs(offX) > padding ? offX : 0), tileY + y - 1)) && offY < -paddingTop) {
							fallDistance = 0;
							offY = -paddingTop;
						}
					}
					...
					if (fallDistance > 0) {
						...
						if ((GameManager.gm.getCollision(tileX + x, tileY  + y + 1) || GameManager.gm.getCollisionNum(tileX +x , tileY + y + 1) == 4 || GameManager.gm.getCollision(tileX + x + (int) Math.signum((int) Math.abs(offX) > padding ? offX : 0), tileY + y + 1)) && offY > 0) {
							fallDistance = 0;
							offY = 0;
							ground = true;
						}
					...
					posX = tileX + x * GameManager.TS + offX;
					posY = tileY + y * GameManager.TS + offY;
